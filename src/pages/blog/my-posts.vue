...
export default {
  ...
  setup(){
    ...
    const { loadMyPosts, post, posts, searchPost } = usePost()
    ...
    watchEffect(async ()=>{
         ...
         let postedBy = userData.value?._id
      await loadMyPosts({limit, skip, postedBy})
        }
    )
    ...
    return{
      amountPost,
      post, posts , pageNumber,pageCount,selectPage,changeAmountOnPage, pageVariants,searchValue, transitionToPost
    }
  }
}
</script>
